...
bool VPNPacketHeader::is_valid() const {
  return version == 1 && payload_length <= 65535 && timestamp > 0;
}
...
VPNPacketType VPNPacket::get_type() const { return header_.type; }

uint32_t VPNPacket::get_sequence_number() const {
  return header_.sequence_number;
}

const std::vector<uint8_t> &VPNPacket::get_payload() const { return payload_; }
...
VPNPacket VPNPacket::deserialize(const std::vector<uint8_t> &data) {
  ...
  std::vector<uint8_t> payload(data.begin() + VPNPacketHeader::HEADER_SIZE,
...
bool VPNPacket::verify_checksum() const {
  ...
  return std::equal(calculated.begin(), calculated.end(),
                    header_.checksum.begin());
}

void VPNPacket::update_checksum() { header_.checksum = calculate_checksum(); }
...
VPNStateMachine::VPNStateMachine()
    : current_state_(VPNConnectionState::DISCONNECTED),
      handshake_timeout_ms_(30000), keepalive_interval_ms_(60000) {
  update_activity();
}
...
VPNConnectionState VPNStateMachine::get_state() const { return current_state_; }
...
void VPNStateMachine::set_state_change_callback(
    std::function<void(VPNConnectionState, VPNConnectionState)> callback) {
...
void VPNStateMachine::handle_handshake_init(const VPNPacket &) {
  if (current_state_ == VPNConnectionState::DISCONNECTED) {
    transition_to(VPNConnectionState::AUTHENTICATING);
  }
}

void VPNStateMachine::handle_handshake_response(const VPNPacket &) {
  if (current_state_ == VPNConnectionState::CONNECTING) {
    transition_to(VPNConnectionState::AUTHENTICATING);
  }
}

void VPNStateMachine::handle_handshake_complete(const VPNPacket &) {
  if (current_state_ == VPNConnectionState::AUTHENTICATING) {
    transition_to(VPNConnectionState::CONNECTED);
  }
}

void VPNStateMachine::handle_data_packet(const VPNPacket &) {
  if (current_state_ != VPNConnectionState::CONNECTED) {
    transition_to(VPNConnectionState::ERROR_STATE);
  }
}

void VPNStateMachine::handle_keepalive(const VPNPacket &) {
  if (current_state_ == VPNConnectionState::CONNECTED) {
    update_activity();
  }
}

void VPNStateMachine::handle_disconnect(const VPNPacket &) {
  transition_to(VPNConnectionState::DISCONNECTED);
}

void VPNStateMachine::handle_error_response(const VPNPacket &) {
  transition_to(VPNConnectionState::ERROR_STATE);
}
...
std::vector<uint8_t> VPNProtocolHandler::encapsulate_ipv6_packet(
    const std::vector<uint8_t> &ipv6_packet) {
  auto vpn_packet = create_data_packet(ipv6_packet);
  return vpn_packet.serialize();
}

std::vector<uint8_t> VPNProtocolHandler::decapsulate_vpn_packet(
    const std::vector<uint8_t> &vpn_packet) {
  try {
...
VPNPacket
VPNProtocolHandler::create_handshake_init(const std::string &client_id) {
...
VPNPacket
VPNProtocolHandler::create_data_packet(const std::vector<uint8_t> &data) {
...
void VPNProtocolHandler::process_incoming_packet(
...
void VPNProtocolHandler::set_packet_handler(
    std::function<void(const VPNPacket &)> handler) {
  packet_handler_ = std::move(handler);
}

void VPNProtocolHandler::set_error_handler(
    std::function<void(const std::string &)> handler) {
  error_handler_ = std::move(handler);
}

bool VPNProtocolHandler::validate_packet_structure(
...
std::vector<VPNPacket>
VPNFragmentationHandler::fragment_packet(const VPNPacket &packet) {
  ...
  size_t total_fragments =
      (payload.size() + max_fragment_size_ - 1) / max_fragment_size_;
  ...
  for (size_t i = 0; i < total_fragments; ++i) {
    ...
    std::vector<uint8_t> fragment_payload(payload.begin() + start,
                                          payload.begin() + end);

    VPNPacket fragment(packet.get_type(), fragment_payload);
    fragment.set_sequence_number(fragment_id);
    fragment.set_flags(static_cast<uint8_t>(i) |
                       (i == total_fragments - 1 ? 0x80 : 0x00));
...
std::optional<VPNPacket>
VPNFragmentationHandler::reassemble_fragments(const VPNPacket &fragment) {
  ...
  if (all_fragments_received(fragment_id)) {
    ...
    std::sort(fragments.begin(), fragments.end(),
              [](const VPNPacket &a, const VPNPacket &b) {
                return (a.get_sequence_number() & 0xFF) <
                       (b.get_sequence_number() & 0xFF);
              });
...
bool VPNFragmentationHandler::all_fragments_received(
