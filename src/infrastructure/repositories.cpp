...
void InMemoryConnectionRepository::store(
    const domain::ConnectionContext &connection) {
  std::unique_lock lock(repository_mutex_);
  connections_[connection.connection_id] = connection;
  update_client_index(connection);
}

std::optional<domain::ConnectionContext>
InMemoryConnectionRepository::find_by_id(domain::ConnectionId id) {
  std::shared_lock lock(repository_mutex_);
  auto it = connections_.find(id);
  return it != connections_.end() ? std::make_optional(it->second)
                                  : std::nullopt;
}

std::vector<domain::ConnectionContext>
InMemoryConnectionRepository::find_by_client(
...
void InMemoryConnectionRepository::update_state(domain::ConnectionId id,
...
std::vector<domain::ConnectionContext>
InMemoryConnectionRepository::get_all_active() {
  std::shared_lock lock(repository_mutex_);
...
void InMemoryConnectionRepository::update_client_index(
    const domain::ConnectionContext &connection) {
  client_connections_[connection.client_id].push_back(connection.connection_id);
}

void InMemoryConnectionRepository::remove_from_client_index(
    domain::ConnectionId id, const domain::ClientId &client_id) {
  auto client_it = client_connections_.find(client_id);
  if (client_it != client_connections_.end()) {
    auto &connections = client_it->second;
    connections.erase(std::remove(connections.begin(), connections.end(), id),
                      connections.end());
...
FileSystemLogger::FileSystemLogger(const std::string &log_file_path,
                                   domain::LogLevel level)
    : log_file_path_(log_file_path), current_level_(level),
...
void FileSystemLogger::write_log(domain::LogLevel level,
...
std::string FileSystemLogger::format_log_entry(domain::LogLevel level,
                                               const std::string &message) {
...
YamlConfigurationProvider::YamlConfigurationProvider(
    ...
    : config_file_path_(config_file_path), auto_reload_enabled_(false),
...
std::string
YamlConfigurationProvider::get_string(const std::string &key,
...
int YamlConfigurationProvider::get_int(const std::string &key,
...
bool YamlConfigurationProvider::get_bool(const std::string &key,
...
YamlConfigurationProvider::get_string_list(const std::string &key) const {
  std::string value = get_string(key);
  if (value.empty())
    return {};
...
void YamlConfigurationProvider::reload() { load_config_file(); }
...
std::string
YamlConfigurationProvider::parse_value(const std::string &raw_value) const {
...
bool YamlConfigurationProvider::parse_bool_value(
...
std::vector<std::string>
YamlConfigurationProvider::parse_list_value(const std::string &value) const {
...
ConcreteSeedGenerator::ConcreteSeedGenerator(domain::SeedStrategy strategy)
    : current_strategy_(strategy), base_seed_(0),
...
domain::SeedValue
ConcreteSeedGenerator::generate(const domain::SeedContext &context) {
...
domain::SeedValue ConcreteSeedGenerator::generate_per_connection(
    const domain::SeedContext &context) {
  return combine_seeds(
      base_seed_,
      hash_string(context.client_id + std::to_string(context.connection_id)));
}

domain::SeedValue
ConcreteSeedGenerator::generate_per_client(const domain::SeedContext &context) {
  auto it = client_seed_cache_.find(context.client_id);
  if (it != client_seed_cache_.end()) {
    return it->second;
  }

  domain::SeedValue client_seed =
      combine_seeds(base_seed_, hash_string(context.client_id));
  client_seed_cache_[context.client_id] = client_seed;
  return client_seed;
}

domain::SeedValue ConcreteSeedGenerator::generate_per_time_window(
    const domain::SeedContext &context) {
  auto now = std::chrono::steady_clock::now();
  auto window =
      std::chrono::duration_cast<std::chrono::hours>(now.time_since_epoch())
          .count();
  return combine_seeds(base_seed_,
                       hash_string(context.client_id + std::to_string(window)));
}

domain::SeedValue
ConcreteSeedGenerator::generate_hybrid(const domain::SeedContext &context) {
...
domain::SeedValue
ConcreteSeedGenerator::hash_string(const std::string &input) const {
  std::hash<std::string> hasher;
  return static_cast<domain::SeedValue>(hasher(input));
}

domain::SeedValue
ConcreteSeedGenerator::combine_seeds(domain::SeedValue seed1,
                                     domain::SeedValue seed2) const {
...
MemoryPoolIPv6Manager::MemoryPoolIPv6Manager(
    const domain::IPv6Address &base_prefix, uint8_t prefix_length,
    size_t initial_pool_size)
    : base_prefix_(base_prefix), prefix_length_(prefix_length),
...
void MemoryPoolIPv6Manager::release(const domain::IPv6Address &address) {
  ...
  auto it = std::find_if(
      seed_to_address_.begin(), seed_to_address_.end(),
...
domain::IPv6Address
MemoryPoolIPv6Manager::seed_to_ipv6_address(domain::SeedValue seed) const {
...
bool MemoryPoolIPv6Manager::is_valid_ipv6_address(
    const domain::IPv6Address &) const {
  return true;
}

bool MemoryPoolIPv6Manager::is_in_prefix_range(
    const domain::IPv6Address &address) const {
...
LinuxNetworkInterface::LinuxNetworkInterface(const std::string &interface_name)
    : interface_name_(interface_name), netlink_socket_(-1),
...
void LinuxNetworkInterface::add_address(const domain::IPv6Address &address) {
  ...
  std::string command =
      "ip -6 addr add " + addr_str + "/128 dev " + interface_name_;
...
void LinuxNetworkInterface::remove_address(const domain::IPv6Address &address) {
  ...
  std::string addr_str = ipv6_to_string(address);
  std::string command =
      "ip -6 addr del " + addr_str + "/128 dev " + interface_name_;

  if (execute_ip_command(command)) {
    configured_addresses_.erase(
        std::remove_if(configured_addresses_.begin(),
...
void LinuxNetworkInterface::add_route(const domain::IPv6Address &dest,
                                      const domain::IPv6Address &gateway) {
  std::lock_guard lock(interface_mutex_);
  std::string dest_str = ipv6_to_string(dest);
  std::string gateway_str = ipv6_to_string(gateway);
  std::string command = "ip -6 route add " + dest_str + " via " + gateway_str +
                        " dev " + interface_name_;
...
void LinuxNetworkInterface::remove_route(const domain::IPv6Address &dest) {
  ...
  std::string dest_str = ipv6_to_string(dest);
  std::string command =
      "ip -6 route del " + dest_str + " dev " + interface_name_;
  execute_ip_command(command);
}
...
bool LinuxNetworkInterface::execute_ip_command(
    const std::string &command) const {
  int result = system(command.c_str());
  return result == 0;
}

std::string LinuxNetworkInterface::ipv6_to_string(
    const domain::IPv6Address &address) const {
...
OpenSSLCryptographyService::~OpenSSLCryptographyService() { cleanup_openssl(); }

std::vector<uint8_t>
OpenSSLCryptographyService::encrypt(const std::vector<uint8_t> &data,
                                    const std::vector<uint8_t> &key) {
  return chacha20_poly1305_encrypt(data, key);
}

std::vector<uint8_t>
OpenSSLCryptographyService::decrypt(const std::vector<uint8_t> &data,
                                    const std::vector<uint8_t> &key) {
...
bool OpenSSLCryptographyService::verify_signature(
...
std::vector<uint8_t> OpenSSLCryptographyService::chacha20_poly1305_encrypt(
  ...
  try {
    ...
    int len;
    if (EVP_EncryptUpdate(ctx, ciphertext.data() + 12, &len, plaintext.data(),
                          plaintext.size()) != 1) {
      throw std::runtime_error("failed to encrypt data");
    }

    int final_len;
    if (EVP_EncryptFinal_ex(ctx, ciphertext.data() + 12 + len, &final_len) !=
        1) {
      throw std::runtime_error("failed to finalize encryption");
    }

    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, 16,
                            ciphertext.data() + 12 + plaintext.size()) != 1) {
...
std::vector<uint8_t> OpenSSLCryptographyService::chacha20_poly1305_decrypt(
  ...
  std::vector<uint8_t> encrypted_data(ciphertext.begin() + 12,
                                      ciphertext.end() - 16);
...
std::vector<uint8_t>
OpenSSLCryptographyService::generate_secure_random(size_t length) {
